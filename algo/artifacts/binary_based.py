...
class BinaryBasedStrategy(ArtifactsCommonMixin):
    def encode(self, context: EncodeContext) -> EncodeResult:
        ...
        if prob[0] == 1:
            ...
            return EncodeResult(sampled_token_id=sampled, bits_consumed=0, metadata={"next_bit_index": context.bit_index})

        bins, prob_new = self._binary_based_recombination(prob, indices, context.precision)
