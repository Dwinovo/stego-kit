...
- 可插拔随机源：支持传入 `PRG`（`stegokit/utils/prg.py`）
...
| ADG | `adg` | 否 | 基于接近 0.5/0.5 的递归动态分组进行 bit 嵌入 |
| Discop | `discop` | 是（`generate_random`） | 基于 Huffman 树与随机路径 |
| Discop Base | `discop_base` | 是（`generate_random`） | Discop 的基线版本 |
| Huffman | `huffman` | 否 | 取 top `2^bit_num` 候选并按 Huffman 前缀码嵌入 bit 串 |
| Meteor | `meteor` | 是（`generate_bits`） | 掩码比特参与编码/解码 |
| ARS | `ars` | 否 | 支持 `regular` 与 `robust` 两种解码 |
| FDPSS Differential Based | `fdpss_differential_based` | 是（`generate_random`） | FDPSS 策略之一 |
| FDPSS Binary Based | `fdpss_binary_based` | 是（`generate_random`） | FDPSS 策略之一 |
| FDPSS Stability Based | `fdpss_stability_based` | 是（`generate_random`） | FDPSS 策略之一 |
...
```bash
pip install -i https://test.pypi.org/simple/ --extra-index-url https://pypi.org/simple stegokit
```
